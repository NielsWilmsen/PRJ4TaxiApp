...
export class MyAuthAuthenticationStrategyProvider implements Provider<AuthenticationStrategy | undefined> {
  constructor(
    ...
    @repository(DriverRepository) private driverRepository: DriverRepository,
  ) {}
  ...
  async verifyToken(
    ...
    try {
      ...
      try {
        const user : Customer = await this.customerRepository.findById(username);
        await this.verifyRoles(username);

        done(null, {name: username, email: user.email, [securityId]: username});
      }catch (e) {
        const user : Driver = await this.driverRepository.findById(username);
        await this.verifyRoles(username);
