...
class ConfirmOrderViewController: UIViewController, ResponseHandler {
    ...
    @IBAction func confirmOrder(_ sender: Any) {
        ...
        let parameters = ["customer_email": order.customerEmail.email, "driver_email": order.driver.email, "fare": "1", "pick_up_point": order.pickup.name!, "destination": order.destination.name!] as [String: String]
